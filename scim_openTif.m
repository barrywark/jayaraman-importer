function varargout = scim_openTif(varargin)
%% function [header,Aout,cmap] = scim_openTif(varargin)
% Opens a TIF file and store its contents as output array Aout, if specified. 
...
% If no flag causing display or file output is present AND Aout is not output, then extracted image data is written to Matlab workspace in same format as Aout would be returned.
%
%% SYNTAX
%   scim_openTif()
...
%   NO ARGUMENTS
%       'header': Specifies that only header is output; useful for getting file info without reading the data
...
flagNames = {'header' 'flat' 'show' 'showmovie' 'showmip' 'cell' 'rgb' 'uselut' 'channel' 'channels' 'slice' 'slices' 'frame' 'frames' 'colororder' 'fourcell' 'fourchan' 'fourcells' 'fourchans' 'allcell' 'allcells' 'allchan' 'allchans'};
argFlags = {'channel' 'channels' 'slice' 'slices' 'frame' 'frames' 'channelorder' 'type' 'write' 'writemip' 'writeavi' 'aspect'};
...
numSlices = header.acq.numberOfZSlices;
...
%Obtain LUT data, if present
if  ~isfield(header.internal,'lowPixelValue1')
    if useLUT
    ...
    acqLUT = cell(1,maxNumChans);
    for i=1:length(acqLUT)
        acqLUT{i} = [header.internal.(['lowPixelValue' num2str(i)]) header.internal.(['highPixelValue' num2str(i)])];
    end
end
...
try
    ...
    for i=1:length(selection)
        ...
        for j = 1:length(savedChans)
            idx = numChans * (selection(i) - 1) + savedChans(j); 
            if forceFlat
                if ismember(savedChans(j), selChans)
                    count = count + 1;
                    if useImreadInfo
                        Aout(:,:,count) = imread(fileName, idx,'Info',info);
                    else
                        Aout(:,:,count) = imread(fileName, idx);
                    end
                end
            elseif forceFour
                if ismember(savedChans(j),selChans)
                    if useImreadInfo
                        Aout(:,:,savedChans(j),i) = imread(fileName, idx,'Info',info);
                    else
                        Aout(:,:,savedChans(j),i) = imread(fileName, idx);
                    end
                end
            elseif forceRGB
                if ismember(savedChans(j),selChans)
                    ...
                    if colorIdx < 4                        
                        if useImreadInfo
                            Aout(:,:,colorIdx,i) = Aout(:,:,colorIdx,i) + scaleImage(imread(fileName, idx,'Info',info),savedChans(j));
                        else
                            Aout(:,:,colorIdx,i) = Aout(:,:,colorIdx,i) + scaleImage(imread(fileName, idx),savedChans(j));                                                        
                        end
                    else
                        if useImreadInfo
                            grayImage = scaleImage(imread(fileName, idx,'Info',info),savedChans(j));
                        else
                            grayImage = scaleImage(imread(fileName, idx),savedChans(j));
                        end
                        for k = 1:3
            ...
            elseif forceCell
                ...
                if ismember(savedChans(j), selChans)
                    if useImreadInfo
                        Aout(:,:,j,i) = imread(fileName, idx,'Info',info);
                    else                        
                        Aout(:,:,j,i) = imread(fileName, idx);
                    end
                end
            end
        end
...
if any(ismember({'write' 'writeavi' 'writemip'},flags))
    
    %Determine filename clause to write, if applicable
    writeFlag = intersect({'write' 'writeavi' 'writemip'},flags);
    ...
    if numSelections == 1 || ismember('write',flags)
        if any(ismember({'writeavi' 'writemip'},flags)) %numSelections = 1 cannot coexist with writeavi/writemip                  
    ...
    switch target
        ...
        case 'avi'
            ...
            if forceRGB
                ...
                movie2avi(M,determineOutFileName(['rgb' dataReductionStr], ext),'fps',fps);                        
            else
                for i=1:length(selChans)
                    [M,fps] = makeMovie(extractChannelData(Aout,selChans(i)),true);
                    movie2avi(M,determineOutFileName(['chan' num2str(selChans(i)) imageReductionStr], ext),'fps',fps);
                end
            end                    
    end
...
if ismember(nargout,[0 1])
    ...
    if any(ismember({'show' 'showmip' 'showmovie' 'write' 'writeavi' 'writemip'},flags)) %Don't write to workspace if image was shown and/or written to file
        clear Aout; %probably not necessary, but sure feels good...
    elseif ismember(fileStem,evalin('base','who')) %Don't overwrite auto-named workspace variable. 
        fprintf(2,['WARNING: Variable ''' fileStem ''' already exists in workspace. Image data was stored to ''Aout'' instead.']);
        assignin('base','Aout',Aout);        
    else
        assignin('base',fileStem,Aout);
        disp(['Stored image data to variable ' fileStem]);
    end
else
    varargout{1} = header;
    varargout{2} = Aout; 
...
%% GENERAL HELPERS
    ...
    function [M,frameRate] = makeMovie(A,forAVI,forShow)
        ...
        if nargin < 2 || ~forAVI || ~ispc
            cmapLength = double(intmax('uint8'));
        else
    ...
    function x = convert2Uint8(x,cmapLength)
        ...
        x = uint8(x/(max(x(:))/(cmapLength-1))); %This avoids conversion to double. Some off-by-one optimizations might yet be done.
    end        
